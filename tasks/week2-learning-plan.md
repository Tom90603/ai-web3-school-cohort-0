# Week 2 个人学习计划

> 学员：Tom90603 | 工具：Hermes Agent | 方向：产品研究
>
> Week 2 主题：AI × Web3 交叉研究与方向选择
>
> 阅读材料：[Handbook → AI × Web3 Bridge](https://aiweb3.school/zh/handbook/bridge/chain-aware-context/)
>
> 不用按天安排，每个模块读完了勾上就行。产出物不用复杂，能说明白就行。

---

## Todo List

### 1. 🌉 Bridge 层全景理解
- [ ] [Chain-aware Context](https://aiweb3.school/zh/handbook/bridge/chain-aware-context/) —— 链上状态如何进入 Agent 上下文
- [ ] [Web3 Tool Use](https://aiweb3.school/zh/handbook/bridge/web3-tool-use/) —— Agent 调用 RPC/钱包/合约的能力
- [ ] [Agent Workflow](https://aiweb3.school/zh/handbook/bridge/agent-workflow/) —— 哪些步骤需要 human-in-the-loop
- [ ] **产出**：一条链路图（Excalidraw / Figma / 手拍都行）——「Agent 接收任务 → 读取链上状态 → 调用工具 → 人工复核 → 签名上链 → 验证结果」

### 2. 🔐 Agent 钱包与权限
- [ ] [Agent Wallet](https://aiweb3.school/zh/handbook/bridge/agent-wallet/)
- [ ] [Wallet & Permission 赛道](https://aiweb3.school/zh/handbook/tracks/wallet-permission/)
- [ ] [Account Abstraction](https://aiweb3.school/zh/handbook/web3/account-abstraction/)（可选加深）
- [ ] **产出**：EOA vs Smart Account vs Session Key 的对比说明

### 3. 💰 机器支付与结算
- [ ] [Machine Payment](https://aiweb3.school/zh/handbook/bridge/machine-payment/)
- [ ] [Settlement & Escrow](https://aiweb3.school/zh/handbook/bridge/settlement-and-escrow/)
- [ ] [Agentic Commerce 赛道](https://aiweb3.school/zh/handbook/tracks/agentic-commerce/)
- [ ] **产出**：整理 2-3 个 PM 视角的思考（机器支付是真需求还是叙事？Gas 费超过支付金额怎么办？）

### 4. 🆔 Agent 身份与信任
- [x] [Agent Identity](https://aiweb3.school/zh/handbook/bridge/agent-identity/)
- [x] [Agent Trust & Reputation](https://aiweb3.school/zh/handbook/bridge/agent-trust-and-reputation/)
- [x] [Verifiable AI](https://aiweb3.school/zh/handbook/bridge/verifiable-ai/)
- [x] **产出**：[传统 vs 区块链对比表](../experiments/identity-trust-verifiable-comparison.md)

### 5. 🧭 Hackathon 方向决定
Week3 启动 Hackathon，这里要定方向了：
- [ ] 快速扫一遍赛道：
  - [Agentic Commerce](https://aiweb3.school/zh/handbook/tracks/agentic-commerce/)
  - [Wallet / Permission](https://aiweb3.school/zh/handbook/tracks/wallet-permission/)
  - [AI Security](https://aiweb3.school/zh/handbook/tracks/ai-security/)
  - [Governance](https://aiweb3.school/zh/handbook/tracks/governance/)
  - [Dev Tooling](https://aiweb3.school/zh/handbook/tracks/dev-tooling/)
- [ ] 挑一个方向，写 100 字说明"为什么选这个"
- [ ] 记到 `experiments/hackathon-direction.md`

### 6. 💡 Portfolio 素材
Week2 读完的材料可以放进作品集，不用特别做什么，心里有数就行：
> 作为 PM/设计师，我关注 AI Agent 从"回答问题"到"执行链上操作"时，交互范式和安全边界的变化——权限设计、支付流程、身份信任，这些都是产品层面绕不开的问题。

---

## 打卡模板

```markdown
📅 共学营打卡 | 2026.05.XX | Week 2

**今日内容：** [读了什么]
**🎯 完成：**
- 
**💡 关键收获：**
- 
**⏱ 用时：** ~Xh
```

---

*生成时间：2026-05-26 | 工具：Hermes Agent*
